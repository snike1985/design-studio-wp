...
function acf_get_select_input( $atts = array() ) {
	...
	return '<select ' . acf_esc_atts( $atts ) . '>' . acf_walk_select_input( $choices, $value ) . '</select>';
...
function acf_walk_select_input( $choices = array(), $values = array(), $depth = 0 ) {
	...
	foreach( $choices as $value => $label ) {
		...
		if( is_array($label) ){
			...
			$html .= '<optgroup label="' . esc_attr($value) . '">' . acf_walk_select_input( $label, $values, $depth+1 ) . '</optgroup>';
